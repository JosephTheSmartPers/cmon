const { Client, Message, EmbedBuilder, SlashCommandBuilder, CommandInteraction, PermissionFlagsBits, UserFlags } = require("discord.js")
...
module.exports = {
    ...
    .setName("userinfo")
    .setDescription("Shows info about a person.")
    .setDMPermission(false)
    .addUserOption(option => option.setName("user").setDescription("The person u wanna scan (leave empty for yourself).")),

    run: async (client, interaction, args) => {

        console.log(UserFlags.BugHunterLevel2)
    ...
    const member = interaction.guild.members.cache.find(gMember => gMember.id == target.id) 
...
statuss = "📱Phone"
               ...
               let embed = new EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        {name: `🧾Roles(${member.roles.cache.size -1})`, value: `${member.roles.cache.filter(r => r.id !== interaction.guild.id).map(roles => `<@&${roles.id}>`).join(" **|** ") || "❌ No Roles"}`, inline: true},
